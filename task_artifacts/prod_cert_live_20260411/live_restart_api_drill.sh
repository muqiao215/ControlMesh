...
ROOT="/root/.ductor/analysis/ductor-src"
ART="$ROOT/task_artifacts/prod_cert_live_20260411/live_drill"
CFG="$HOME/.ductor/config/config.json"
...
wait_active() {
  ...
  for _ in $(seq 1 "$tries"); do
    if systemctl --user is-active --quiet ductor; then
      ok=1
      break
    fi
    sleep 1
...
restore_config() {
  if [[ -f "$BACKUP" ]]; then
    ...
    wait_active 90
    systemctl --user status ductor --no-pager > "$ART/final_restore_status.log" 2>&1 || true
    systemctl --user show ductor \
...
cfg = Path.home() / ".ductor/config/config.json"
...
systemctl --user status ductor --no-pager > "$ART/pre_restart_status.log" 2>&1 || true
systemctl --user show ductor \
  -p ActiveState -p SubState -p ExecMainPID -p NRestarts -p Result -p ActiveEnterTimestamp \
  --no-pager > "$ART/pre_restart_show.log" 2>&1 || true

log "restarting live ductor service"
systemctl --user restart ductor
wait_active 90
sleep 3
systemctl --user status ductor --no-pager > "$ART/post_restart_status.log" 2>&1 || true
systemctl --user show ductor \
  -p ActiveState -p SubState -p ExecMainPID -p NRestarts -p Result -p ActiveEnterTimestamp \
...
ductor api enable > "$ART/api_enable.log" 2>&1
log "restarting after API enable"
systemctl --user restart ductor
wait_active 90
wait_port_state 8741 open 90

systemctl --user status ductor --no-pager > "$ART/post_api_enable_status.log" 2>&1 || true
systemctl --user show ductor \
  -p ActiveState -p SubState -p ExecMainPID -p NRestarts -p Result -p ActiveEnterTimestamp \
