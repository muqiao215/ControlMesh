...
- `systemctl_show.log`
- `ductor_status.log`
- `ductor_service_status.log`
- `ductor_service_unit.log`
- `config_summary.log`
- `env_key_presence.log`
- `port_scan.log`
- `internal_health.json`
- `agent_log_tail.log`
- `installed_ductor_service_logs.log`
- `repo_ductor_service_logs.log`
...
### 1. Can a real supervised ductor deployment be brought up stably from zero to running under the intended service/supervisor path?

Partially proven, but not enough for prod certification.

Proven live:

- The real user-systemd service is installed and active: `systemctl_show.log` records `ActiveState=active`, `SubState=running`, `ExecMainPID=503798`, `Restart=always`, `RestartUSec=5s`, `FragmentPath=/root/.config/systemd/user/ductor.service`, and `ActiveEnterTimestamp=Sat 2026-04-11 04:40:52 UTC`.
- The intended supervisor environment is present: `Environment=... HOME=/root IS_SANDBOX=1 DUCTOR_SUPERVISOR=1`.
- `ductor_status.log` reports the live process running with `pid=503798`, provider `codex (gpt-5.4)`, `Errors: 0`, and the real home/config/workspace/log paths under `/root/.ductor`.
...
- I did not run a fresh `ductor service install` or destructive clean-home reinstall on the live host during this certification pass.
...
- `live_drill/post_restart_status.log` and `live_drill/post_restart_show.log` prove one controlled `systemctl --user restart ductor` succeeded and the service came back with a new PID (`507548`) in `active/running`.
...
- `agent_log_tail.log` shows real Telegram ingress and provider execution: `Message received text=返回了吗`, `Codex CLI wrapper: cwd=/root/.ductor/workspace, model=gpt-5.4`, `Codex subprocess starting`, and `Streaming flow completed`.
...
- File logs are available at `/root/.ductor/logs/agent.log`; `agent_log_tail.log` contains startup, transport, provider, task, and failure-localization evidence.
- `systemctl status` exposes recent service logs and live process tree; see `systemctl_status.log` and `ductor_service_status.log`.
- The internal supervisor health endpoint responds; see `internal_health.json`.

Observed blocker:

- The deployed `ductor service logs` command is not sufficient on this host. `installed_ductor_service_logs.log` shows only:
...
- `ductor_bot/infra/service_logs.py` now falls back to recent file logs when the user journal is unavailable or `journalctl` is missing.
- `ductor_bot/infra/service_linux.py` passes the resolved `logs_dir` into that fallback path.
- `tests/infra/test_service_linux.py` and `tests/infra/test_service_logs.py` cover the fallback.
- `repo_ductor_service_logs.log` proves the repo version now prints `No user journal available; showing recent file logs instead.` and then shows `agent.log` lines.

Still insufficient:

- The fix has not been deployed into the currently running `/root/.local/bin/ductor` service binary.
...
- That same follow-up drill still did not complete the API smoke because `live_drill/api_enable.log` shows `ductor: command not found` inside the drill shell before the temporary API enable/restart/health/catalog sequence could finish.
...
- One bounded live `systemctl --user restart ductor` drill now shows the service can come back under the user-systemd supervisor path.

Exact missing evidence layer:

- Deploy the service-log fallback fix to the live service binary, then re-run `ductor service logs` against the deployed command, not only `uv run ductor service logs` from the repo.
...
```bash
uv run ruff check ductor_bot/infra/service_logs.py ductor_bot/infra/service_linux.py tests/infra/test_service_linux.py tests/infra/test_service_logs.py
```

Result: `All checks passed!`.

```bash
uv run mypy ductor_bot/infra/service_logs.py ductor_bot/infra/service_linux.py
```
...
```bash
timeout 3 uv run ductor service logs
```

Result: fallback succeeded and displayed recent `agent.log` lines; see `repo_ductor_service_logs.log`.

Failed or insufficient:

```bash
timeout 3 ductor service logs
```

Result: the currently deployed command showed `No journal files were found.`; see `installed_ductor_service_logs.log`.
...
- `ductor_bot/infra/service_logs.py`
- `ductor_bot/infra/service_linux.py`
