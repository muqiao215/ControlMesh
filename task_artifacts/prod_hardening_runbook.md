...
```bash
export DUCTOR_HOME=/path/to/ductor-home
python - <<'PY'
...
1. `ductor` or `python -m ductor_bot` starts the bot process.
2. `load_config()` resolves `DUCTOR_HOME`, creates/merges `config/config.json`, and initializes the workspace.
3. `create_orchestrator()` resolves `DuctorPaths`, injects runtime environment, starts observers, and starts the direct API when `api.enabled=true`.
...
- Linux: systemd user service, logs through `journalctl --user -u ductor -f --no-hostname`.
- macOS: launchd Launch Agent, recent logs from `~/.ductor/logs/agent.log`.
- Windows: Task Scheduler task, recent logs from `~/.ductor/logs/agent.log`.
...
print(json.loads(Path("~/.ductor/config/config.json").expanduser().read_text())["api"]["token"])
PY
)"
...
- Linux: follows `journalctl --user -u ductor -f --no-hostname`.
- macOS/Windows: prints recent lines from `~/.ductor/logs/agent.log`, falling back to newest `*.log`.
...
```bash
rm ~/.ductor/workspace/.history/index.sqlite3
...
- Full `ductor` supervised startup with live Telegram/Matrix/Feishu credentials and at least one authenticated provider CLI was not executed in this repo-only cut.
- `ductor service install/start/logs` was not exercised on an installed service; behavior is documented from code paths only.
