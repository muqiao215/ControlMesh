...
|---|---|---|---|
| Task routing and job creation | tasks/host_execution.py; tasks/hub.py:_start_host_job_task | Explicit host registration plus imported approved jobs works; workunit routing/create flow missing | Normal task creates exactly one bound job; source restrictions checked before creation/launch |
| Step advancement | runtime/host_jobs.py:_advance_job | Explicit approval and a separate task per step tested; automatic advancement absent | Ordered automatic progression for authorized steps, stop at approval/failed dependency, restart without duplicate start |
| Explicit cancellation | runtime/host_jobs.py:cancel | Running-step cancellation now retains same-episode outcome and synchronizes cancelled job/step; queued-step cancellation is atomic for current approvals; crash-window convergence remains | Atomic cancellation intent plus confirmed process outcome reflected in job/step; no success claim or repeated command |
...
| Logs and exit artifacts | runtime/host_jobs.py:stdout_path/stderr_path/exit_code_path | Bounded effect_observations retained; no host log paging/control or durable streamed logs | Authorized bounded stdout/stderr retrieval during/after run, restart persistence, explicit output-limit behavior |
