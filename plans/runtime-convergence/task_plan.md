...
Continue from `TaskIngress`, `OpenCodeWorker`, `NativeReconciler` and the authenticated `DeviceCoordinator`: connect the now-ported source/grant policy to actual sandbox/provider launchers and the remote worker port, preserving worker origin separately from task provenance. The device adapter still needs pre-dispatch native preparation, durable manifest/observation transport and its device-local preflight/native checks. Cover remaining recovery/abandonment cases; integrate independent SpecMesh gates with current checkout snapshots. Complete remaining ownership/side-effect parity against `python-ownership.json` (512 Python modules, 57 serialized TaskEntry fields). Do not activate over live data before writer-exclusion and recovery gates.
