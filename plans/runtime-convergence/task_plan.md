...
| CM-R3 | Provider/transport/workspace/grant ports | Actual adapter smoke for each supported provider; grants enforced natively; process tree cleanup and result-delivery reconciliation | in_progress: source/grant/reply-policy and one-shot host process ports; real local OpenCode profile; native provider matrix, sandbox and transports pending |
...
Check exact-commit one-shot publication CI, then port the actual container process lifecycle
beyond host supervision and the qualified read-only OpenCode profile.
