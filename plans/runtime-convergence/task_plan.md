...
Continue from `OpenCodeWorker`, `NativeReconciler` and the authenticated `DeviceCoordinator`: connect production provider/grant/source profiles to the remote worker port, and cover remaining recovery/abandonment cases. Integrate independent SpecMesh gates with current checkout snapshots. Complete the remaining ownership/side-effect audit against `python-ownership.json` (512 Python modules, 57 serialized TaskEntry fields); the inventory itself does not establish parity. Do not activate over live data before writer-exclusion and recovery gates.
