...
from controlmesh.config import AgentConfig, ApiConfig
...
class TestMergeSubAgentConfig:
    ...
    def test_controlmesh_home_always_set_to_agent_home(self) -> None:
        """controlmesh_home is always the agent's home dir, not main's."""
        main = self._main_config()
        sub = SubAgentConfig(name="sub1", telegram_token="sub-token")
        result = merge_sub_agent_config(main, sub, Path("/agents/sub1"))

        assert result.controlmesh_home == "/agents/sub1"
