...
class TestCascadingFallback:
    ...
    async def test_fallback_logs_warning(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("WARNING", logger="controlmesh.bus.bus"):
            await bus.submit(env)
    ...
    async def test_fallback_delivery_failure_is_logged(
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("ERROR", logger="controlmesh.bus.bus"):
            await bus.submit(env)
...
class TestAdapterTransportParam:
    def test_from_cron_result_includes_transport(self) -> None:
        ...
        from controlmesh.bus.adapters import from_cron_result
    ...
    def test_from_heartbeat_includes_transport(self) -> None:
        ...
        from controlmesh.bus.adapters import from_heartbeat
...
# -- Executor CONTROLMESH_TRANSPORT env var --


class TestExecutorTransportEnv:
    def test_build_subprocess_env_includes_transport(self) -> None:
        """_build_subprocess_env sets CONTROLMESH_TRANSPORT from CLIConfig."""
        ...
        assert env["CONTROLMESH_TRANSPORT"] == "mx"

    def test_build_subprocess_env_default_transport(self) -> None:
        """_build_subprocess_env defaults CONTROLMESH_TRANSPORT to 'tg'."""
        from controlmesh.cli.base import CLIConfig
        from controlmesh.cli.executor import _build_subprocess_env
        ...
        env = _build_subprocess_env(config)
        assert env is not None
        assert env["CONTROLMESH_TRANSPORT"] == "tg"
