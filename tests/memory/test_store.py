...
from ductor_bot.memory.dreaming import load_checkpoints, load_sweep_state
...
def test_initialize_memory_v2_bootstraps_layout(tmp_path: Path) -> None:
    ...
    assert "Ductor Memory v2" in paths.authority_memory_path.read_text(encoding="utf-8")
    assert "Ductor Dream Diary" in paths.dream_diary_path.read_text(encoding="utf-8")
