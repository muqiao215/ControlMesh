...
_INIT_RESPONSE = json.dumps(
    {
        "id": 1,
        "result": {"userAgent": "controlmesh/0.98.0"},
    }
)
...
async def test_discover_models_parses_response() -> None:
    proc = _mock_process()
    with (
        patch("controlmesh.cli.codex_discovery.which", return_value="/usr/bin/codex"),
...
async def test_discover_models_codex_not_installed() -> None:
    with patch("controlmesh.cli.codex_discovery.which", return_value=None):
        models = await discover_codex_models()
    assert models == []
