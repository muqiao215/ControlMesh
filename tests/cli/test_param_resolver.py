...
from controlmesh.cli.param_resolver import (
    TaskOverrides,
    resolve_cli_config,
)
...
def test_resolve_invalid_claude_model(
    ...
    with pytest.raises(ControlMeshError, match="Invalid Claude model"):
...
def test_resolve_invalid_codex_model(
    ...
    with pytest.raises(ControlMeshError, match="Invalid Codex model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_gemini_invalid_against_discovered_models(
    ...
    with pytest.raises(ControlMeshError, match="Invalid Gemini model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
