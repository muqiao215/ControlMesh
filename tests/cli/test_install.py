"""Tests for controlmesh install CLI command."""

from __future__ import annotations

from unittest.mock import MagicMock, patch

from controlmesh.cli_commands.install import (
    _install_extra,
    _is_installed,
    cmd_install,
    print_install_help,
...
class TestCmdInstall:
    ...
    def test_no_args_shows_help(self) -> None:
        with patch("controlmesh.cli_commands.install.print_install_help") as mock_help:
            cmd_install([])
    ...
    def test_valid_extra_calls_install(self) -> None:
        with patch("controlmesh.cli_commands.install._install_extra") as mock_install:
...
class TestInstallExtra:
    ...
    def test_already_installed_shows_message(self) -> None:
        with patch("controlmesh.cli_commands.install._is_installed", return_value=True):
            console = MagicMock()
    ...
    def test_not_installed_runs_subprocess_pip(self) -> None:
        with (
            patch("controlmesh.cli_commands.install._is_installed", return_value=False),
            patch(
                "controlmesh.cli_commands.install.detect_install_mode",
                return_value="pip",
            ),
            patch("controlmesh.cli_commands.install.subprocess.run") as mock_run,
            patch("controlmesh.cli_commands.install.Console", return_value=MagicMock()),
        ):
            mock_run.return_value = MagicMock(returncode=0, stderr="")
            _install_extra("matrix")
            mock_run.assert_called_once()
            cmd = mock_run.call_args[0][0]
            assert "controlmesh[matrix]" in cmd
    ...
    def test_failed_install_shows_error(self) -> None:
        with (
            ...
            patch("controlmesh.cli_commands.install.subprocess.run") as mock_run,
            patch("controlmesh.cli_commands.install.Console", return_value=MagicMock()) as console,
        ):
