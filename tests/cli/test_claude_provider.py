...
from controlmesh.cli.claude_provider import (
    ClaudeCodeCLI,
    _add_opt,
    _log_cmd,
    _parse_response,
...
_EXEC_PATH = "controlmesh.cli.executor.asyncio.create_subprocess_exec"
...
class TestInit:
    def test_find_cli_not_found_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("controlmesh.cli.claude_provider.which", lambda _: None)
...
class TestLogCmd:
    def test_truncates_long_values_after_flags(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.INFO, logger="controlmesh.cli.claude_provider"):
            _log_cmd(cmd)
