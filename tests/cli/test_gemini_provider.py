...
if TYPE_CHECKING:
    import pytest
from controlmesh.cli.gemini_provider import GeminiCLI, _log_cmd, _parse_response
...
def _make_cli(monkeypatch: pytest.MonkeyPatch, **overrides: Any) -> GeminiCLI:
    monkeypatch.setattr("controlmesh.cli.gemini_provider.find_gemini_cli", lambda: "/usr/bin/gemini")
    monkeypatch.setattr("controlmesh.cli.gemini_provider.find_gemini_cli_js", lambda: None)
    return GeminiCLI(
        CLIConfig(
            provider="gemini",
...
class TestPrepareEnv:
    def test_prepends_cli_parent_when_cli_path_is_absolute(
        ...
        monkeypatch.setattr(
            "controlmesh.cli.gemini_provider.find_gemini_cli",
            lambda: "/opt/node/v22.0.0/bin/gemini",
        )
        monkeypatch.setattr(
            "controlmesh.cli.gemini_provider.find_gemini_cli_js",
    ...
    def test_host_to_container_path_normalizes_windows_separators(
        ...
        fake_paths = type("P", (), {"controlmesh_home": Path(r"C:\Users\ZOZN109\.controlmesh")})()
        monkeypatch.setattr(
            "controlmesh.cli.gemini_provider.resolve_paths",
            lambda: fake_paths,
        )

        result = GeminiCLI._host_to_container_path(
            r"C:\Users\ZOZN109\.controlmesh\tmp\gemini_system_abc.md"
        )

        assert result == "/controlmesh/tmp/gemini_system_abc.md"
