...
def _make_cli(monkeypatch: pytest.MonkeyPatch, **overrides: Any) -> CodexCLI:
    monkeypatch.setattr("controlmesh.cli.codex_provider.which", lambda _: "/usr/bin/codex")
    return CodexCLI(
        CLIConfig(
            provider="codex",
...
class TestInit:
    def test_find_cli_raises_when_not_on_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("controlmesh.cli.codex_provider.which", lambda _: None)
    ...
    def test_find_cli_uses_resolved_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("controlmesh.cli.codex_provider.which", lambda _: "/opt/bin/codex")
...
class TestLogCmd:
    def test_short_values_not_truncated(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.INFO, logger="controlmesh.cli.codex_provider"):
...
class TestDockerIntegration:
    ...
    async def test_send_with_docker_container_keeps_stdin_open_on_windows(
        ...
        monkeypatch.setattr("controlmesh.cli.codex_provider._IS_WINDOWS", True)
        cli = CodexCLI(
            CLIConfig(
                provider="codex",
                model="gpt-5.2-codex",
...
class TestEdgeCases:
    ...
    def test_parse_output_error_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.ERROR, logger="controlmesh.cli.codex_provider"):
