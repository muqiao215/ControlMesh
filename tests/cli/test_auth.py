...
def test_check_opencode_auth_config_file_top_level_env_anthropic_auth_token(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import controlmesh.cli.auth as _auth_mod
...
def test_check_gemini_auth_controlmesh_config_key(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import controlmesh.cli.auth as _auth_mod
    ...
    controlmesh_config = tmp_path / ".controlmesh" / "config" / "config.json"
    controlmesh_config.parent.mkdir(parents=True)
    controlmesh_config.write_text('{"gemini_api_key":"from-controlmesh-config"}')

    result = check_gemini_auth()

    assert result.status == AuthStatus.AUTHENTICATED
    assert result.auth_file == controlmesh_config


def test_check_gemini_auth_controlmesh_config_null_string_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import controlmesh.cli.auth as _auth_mod
    ...
    controlmesh_config = tmp_path / ".controlmesh" / "config" / "config.json"
    controlmesh_config.parent.mkdir(parents=True)
    controlmesh_config.write_text('{"gemini_api_key":"null"}')
