...
from controlmesh.cleanup.observer import CleanupObserver, _delete_old_files
from controlmesh.config import AgentConfig, CleanupConfig
...
def _make_paths(tmp_path: Path) -> ControlMeshPaths:
    return ControlMeshPaths(controlmesh_home=tmp_path)
...
async def test_maybe_run_skips_wrong_hour(tmp_path: Path) -> None:
    ...
    with patch("controlmesh.cleanup.observer.datetime") as mock_dt:
