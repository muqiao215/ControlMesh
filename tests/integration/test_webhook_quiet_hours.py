...
from controlmesh.config import AgentConfig, HeartbeatConfig, WebhookConfig
...
def _make_manager(paths: ControlMeshPaths) -> WebhookManager:
...
def _add_hook(mgr: WebhookManager, paths: ControlMeshPaths, **overrides: Any) -> WebhookEntry:
    defaults: dict[str, Any] = {
        "id": "test-hook",
        "title": "Test Hook",
        "description": "Testing",
...
async def test_webhook_ignores_heartbeat_quiet_hours(tmp_path: Path) -> None:
    ...
    with patch("controlmesh.cron.execution.build_cmd", return_value=None):
