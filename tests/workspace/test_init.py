...
def _setup_home_defaults(fw_root: Path) -> None:
    ...
    Mirrors the repo ``workspace/`` structure (1:1 copy of ~/.controlmesh/).
    """
    ws = fw_root / "workspace"

    # Top-level CLAUDE.md (controlmesh home)
    ws.mkdir(parents=True)
    (ws / "CLAUDE.md").write_text("# ControlMesh Home CLAUDE.md")
...
def test_controlmesh_home_claude_md_overwritten(tmp_path: Path) -> None:
    """The controlmesh_home/CLAUDE.md is Zone 2 (always overwritten)."""
    paths = _make_paths(tmp_path)
    init_workspace(paths)

    home_claude = paths.controlmesh_home / "CLAUDE.md"
...
def test_inject_docker_notice(tmp_path: Path) -> None:
    paths = _make_paths(tmp_path)
    init_workspace(paths)
    inject_runtime_environment(paths, docker_container="controlmesh-sandbox")
    content = (paths.workspace / "CLAUDE.md").read_text()
    assert "DOCKER CONTAINER" in content
    assert "controlmesh-sandbox" in content
