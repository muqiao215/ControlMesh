...
@pytest.fixture
def mock_paths(tmp_path: Path) -> ControlMeshPaths:
    """Create ControlMeshPaths with temp directories for testing."""
    home_defaults = tmp_path / "home_defaults"
    controlmesh_home = tmp_path / "controlmesh_home"
    ...
    paths = MagicMock(spec=ControlMeshPaths)
    paths.home_defaults = home_defaults
    paths.controlmesh_home = controlmesh_home

    return paths


def test_variant_selection_claude_only(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_codex_only(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_both(mock_paths: ControlMeshPaths) -> None:
...
def test_template_discovery(mock_paths: ControlMeshPaths) -> None:
...
def test_deploy_claude_only_no_agents_md(mock_paths: ControlMeshPaths) -> None:
    ...
    with patch("controlmesh.cli.auth.check_all_auth", return_value=auth):
        ...
        config_claude = mock_paths.controlmesh_home / "config" / "CLAUDE.md"
        assert config_claude.exists()
        assert "Claude Only Template" in config_claude.read_text()

        # Check that AGENTS.md was NOT created
        config_agents = mock_paths.controlmesh_home / "config" / "AGENTS.md"
        assert not config_agents.exists()


def test_deploy_codex_only_with_agents_md(mock_paths: ControlMeshPaths) -> None:
...
def test_deploy_both_with_both_files(mock_paths: ControlMeshPaths) -> None:
...
def test_deploy_all_directories(mock_paths: ControlMeshPaths) -> None:
    ...
    with patch("controlmesh.cli.auth.check_all_auth", return_value=auth):
        ...
        assert (mock_paths.controlmesh_home / "config" / "CLAUDE.md").exists()
        assert (mock_paths.controlmesh_home / "config" / "AGENTS.md").exists()
        assert (mock_paths.controlmesh_home / "workspace" / "cron_tasks" / "CLAUDE.md").exists()
        assert (mock_paths.controlmesh_home / "workspace" / "cron_tasks" / "AGENTS.md").exists()
        assert (
            mock_paths.controlmesh_home / "workspace" / "tools" / "webhook_tools" / "CLAUDE.md"
        ).exists()
        assert (
            mock_paths.controlmesh_home / "workspace" / "tools" / "webhook_tools" / "AGENTS.md"
        ).exists()


def test_fallback_to_static_template(mock_paths: ControlMeshPaths) -> None:
    ...
    with patch("controlmesh.cli.auth.check_all_auth", return_value=auth):
        ...
        deployed = mock_paths.controlmesh_home / "static_test" / "CLAUDE.md"
...
def test_cleanup_removes_agents_md_when_only_claude(mock_paths: ControlMeshPaths) -> None:
    ...
    old_agents1 = mock_paths.controlmesh_home / "config" / "AGENTS.md"
    # cron_tasks files are user-owned — they must survive cleanup
    cron_task_agents = mock_paths.controlmesh_home / "workspace" / "cron_tasks" / "my-task" / "AGENTS.md"
...
def test_cleanup_removes_claude_md_when_only_codex(mock_paths: ControlMeshPaths) -> None:
    """Test that stale CLAUDE.md files are removed when only Codex is authenticated.

    Files inside workspace/cron_tasks/ are user-owned and must NOT be deleted.
    """
    old_claude1 = mock_paths.controlmesh_home / "config" / "CLAUDE.md"
    cron_task_claude = mock_paths.controlmesh_home / "workspace" / "cron_tasks" / "my-task" / "CLAUDE.md"
...
def test_cleanup_keeps_both_when_both_authenticated(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_gemini_only(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_claude_and_gemini(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_all_three(mock_paths: ControlMeshPaths) -> None:
...
def test_variant_selection_codex_and_gemini(mock_paths: ControlMeshPaths) -> None:
...
def test_deploy_with_gemini_creates_gemini_md(mock_paths: ControlMeshPaths) -> None:
    ...
    with patch("controlmesh.cli.auth.check_all_auth", return_value=auth):
        ...
        gemini_md = mock_paths.controlmesh_home / "config" / "GEMINI.md"
        assert gemini_md.exists()
        assert "Gemini Only Template" in gemini_md.read_text()

        # CLAUDE.md and AGENTS.md should NOT exist
        assert not (mock_paths.controlmesh_home / "config" / "CLAUDE.md").exists()
        assert not (mock_paths.controlmesh_home / "config" / "AGENTS.md").exists()


def test_deploy_all_three_providers(mock_paths: ControlMeshPaths) -> None:
    ...
    with patch("controlmesh.cli.auth.check_all_auth", return_value=auth):
        ...
        config_dir = mock_paths.controlmesh_home / "config"
        assert (config_dir / "CLAUDE.md").exists()
        assert (config_dir / "AGENTS.md").exists()
        assert (config_dir / "GEMINI.md").exists()


def test_cleanup_removes_gemini_md_when_not_authenticated(mock_paths: ControlMeshPaths) -> None:
    """Test that stale GEMINI.md files are removed when Gemini is not authenticated."""
    old_gemini = mock_paths.controlmesh_home / "config" / "GEMINI.md"
