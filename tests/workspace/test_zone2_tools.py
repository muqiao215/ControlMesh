...
from controlmesh.workspace.init import _walk_and_copy
...
def temp_workspace(tmp_path: Path):
    ...
    return home_defaults, controlmesh_home
...
def test_cron_tools_py_files_are_zone2(temp_workspace):
    ...
    home_defaults, controlmesh_home = temp_workspace
    ...
    _walk_and_copy(home_defaults, controlmesh_home)

    deployed_tool = controlmesh_home / "workspace" / "tools" / "cron_tools" / "cron_add.py"
...
def test_webhook_tools_py_files_are_zone2(temp_workspace):
    ...
    deployed_tool = controlmesh_home / "workspace" / "tools" / "webhook_tools" / "webhook_add.py"
    assert deployed_tool.exists()
    assert deployed_tool.read_text() == "# version 1"
...
def test_user_tools_py_files_are_zone3(temp_workspace):
    ...
    deployed_tool = controlmesh_home / "workspace" / "tools" / "user_tools" / "custom_tool.py"
    assert deployed_tool.exists()
    assert deployed_tool.read_text() == "# version 1"
...
def test_non_py_files_in_tool_dirs_are_zone3(temp_workspace):
    ...
    deployed_config = controlmesh_home / "workspace" / "tools" / "cron_tools" / "config.json"
...
def test_shared_py_is_also_zone2(temp_workspace):
    ...
    deployed_shared = controlmesh_home / "workspace" / "tools" / "cron_tools" / "_shared.py"
