...
class TestTransportRegistry:
    ...
    def test_telegram_transport(self) -> None:
        ...
        config.is_multi_transport = False
        fake_bot = MagicMock()
        with patch("controlmesh.messenger.telegram.app.TelegramBot", return_value=fake_bot):
    ...
    def test_matrix_transport(self) -> None:
        ...
        config.is_multi_transport = False
        fake_bot = MagicMock()
        with patch("controlmesh.messenger.matrix.bot.MatrixBot", return_value=fake_bot):
            bot = create_bot(config, agent_name="test")
        assert bot is fake_bot

    def test_feishu_transport(self) -> None:
        config = MagicMock()
