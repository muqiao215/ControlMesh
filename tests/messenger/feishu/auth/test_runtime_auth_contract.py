...
def _import_runtime_auth_module() -> ModuleType:
    try:
        return importlib.import_module("ductor_bot.messenger.feishu.auth.runtime_auth")
    except ModuleNotFoundError as exc:  # pragma: no cover - red-path contract
        msg = "missing runtime auth seam module: ductor_bot.messenger.feishu.auth.runtime_auth"
...
def test_persist_device_flow_auth_round_trip_records_minimal_contract(tmp_path: Path) -> None:
    ...
    stored = module.load_device_flow_auth(ductor_home=tmp_path, app_id="cli_123")
...
def test_clear_device_flow_auth_removes_only_persisted_device_flow_record(tmp_path: Path) -> None:
    ...
    module.clear_device_flow_auth(ductor_home=tmp_path, app_id="cli_123")

    assert module.load_device_flow_auth(ductor_home=tmp_path, app_id="cli_123") is None
