...
def _make_orchestrator(
    *,
    ...
    paths.controlmesh_home = Path("/tmp/test-controlmesh")
...
class TestCancelTask:
    async def test_cancel_task_none(self) -> None:
        from controlmesh.messenger.telegram.app import _cancel_task
...
class TestOnHelp:
    ...
    @patch("controlmesh.messenger.telegram.app.send_rich", new_callable=AsyncMock)
    async def test_help_keeps_advanced_commands_discoverable(self, mock_send: AsyncMock) -> None:
        tg_bot, _bot_instance = _make_tg_bot()
        msg = _make_message(chat_id=42)
...
class TestOnStart:
    @patch("controlmesh.messenger.telegram.app.send_rich", new_callable=AsyncMock)
    @patch("controlmesh.messenger.telegram.app.build_welcome_keyboard")
    @patch("controlmesh.messenger.telegram.app.build_welcome_text", return_value="Welcome!")
...
class TestSendWelcomeImage:
    async def test_returns_false_when_no_image_file(self) -> None:
        ...
        with patch(
            "controlmesh.messenger.telegram.app._WELCOME_IMAGE", Path("/nonexistent/file.png")
        ):
    ...
    async def test_returns_true_with_short_caption(self) -> None:
        ...
        with (
            patch("controlmesh.messenger.telegram.app._WELCOME_IMAGE") as mock_path,
            patch(
                "controlmesh.messenger.telegram.app.markdown_to_telegram_html",
                return_value="<b>Hi</b>",
            ),
        ):
    ...
    async def test_returns_false_on_bad_request(self) -> None:
        ...
        with patch("controlmesh.messenger.telegram.app._WELCOME_IMAGE") as mock_path:
            mock_path.is_file.return_value = True
...
class TestOnMessage:
    ...
    @patch("controlmesh.messenger.telegram.app.strip_mention", return_value="clean text")
...
class TestResolveText:
    ...
    @patch("controlmesh.messenger.telegram.app.is_media_addressed", return_value=False)
    @patch("controlmesh.messenger.telegram.app.has_media", return_value=True)
    ...
    @patch("controlmesh.messenger.telegram.app.resolve_media_text", new_callable=AsyncMock)
    @patch("controlmesh.messenger.telegram.app.is_media_addressed", return_value=True)
    @patch("controlmesh.messenger.telegram.app.has_media", return_value=True)
    ...
    @patch("controlmesh.messenger.telegram.app.is_command_for_others", return_value=True)
    @patch("controlmesh.messenger.telegram.app.has_media", return_value=True)
...
class TestCallbackQueryHandler:
    ...
    async def test_model_selector_callback_edits_message(self) -> None:
        ...
        with patch(
            "controlmesh.orchestrator.selectors.model_selector.handle_model_callback",
    ...
    async def test_cron_selector_callback_edits_message(self) -> None:
        ...
        with patch(
            "controlmesh.orchestrator.selectors.cron_selector.handle_cron_callback",
            new_callable=AsyncMock,
            return_value=resp,
        ):
...
class TestHandleModelSelector:
    ...
    async def test_suppresses_bad_request(self) -> None:
        from controlmesh.orchestrator.selectors.models import SelectorResponse
        from controlmesh.session.key import SessionKey
...
class TestCommandHandlers:
    @patch(
        "controlmesh.messenger.telegram.app.handle_abort", new_callable=AsyncMock, return_value=True
    )
    ...
    @patch("controlmesh.messenger.telegram.app.handle_command", new_callable=AsyncMock)
    ...
    @patch("controlmesh.messenger.telegram.app.handle_new_session", new_callable=AsyncMock)
...
class TestWebhookWake:
    async def test_calls_handle_message_and_sends_result(self) -> None:
        import controlmesh.messenger.telegram.transport as _tgt
...
class TestWatchRestartMarker:
    async def test_detects_marker_and_stops(self, tmp_path: Path) -> None:
        ...
        tg_bot, _ = _make_tg_bot()
        orch = _make_orchestrator()
        orch.paths.controlmesh_home = tmp_path
        tg_bot._orchestrator = orch
...
class TestForumTopicPropagation:
    ...
    async def test_on_restart_passes_thread_id(self, mock_send: AsyncMock) -> None:
        ...
        with patch("controlmesh.infra.restart.write_restart_sentinel"):
            await tg_bot._on_restart(msg)
