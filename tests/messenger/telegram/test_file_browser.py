"""Tests for the interactive ~/.controlmesh file browser."""
...
@pytest.fixture
def paths(tmp_path: Path) -> ControlMeshPaths:
    """ControlMeshPaths with a temporary controlmesh_home populated with test dirs/files."""
    ...
    return ControlMeshPaths(controlmesh_home=home)
...
class TestFileBrowserStart:
    async def test_root_listing_shows_directories(self, paths: ControlMeshPaths) -> None:
    ...
    async def test_root_listing_shows_files(self, paths: ControlMeshPaths) -> None:
        text, _ = await file_browser_start(paths)

        assert "sessions.json" in text

    async def test_root_listing_excludes_hidden(self, paths: ControlMeshPaths) -> None:
        text, _ = await file_browser_start(paths)

        assert ".hidden_file" not in text
        assert ".hidden_dir" not in text

    async def test_root_has_no_back_button(self, paths: ControlMeshPaths) -> None:
    ...
    async def test_root_has_file_request_button(self, paths: ControlMeshPaths) -> None:
        _, keyboard = await file_browser_start(paths)
    ...
    async def test_root_has_folder_buttons(self, paths: ControlMeshPaths) -> None:
        _, keyboard = await file_browser_start(paths)
...
class TestDirectoryNavigation:
    async def test_navigate_to_subfolder(self, paths: ControlMeshPaths) -> None:
        text, keyboard, prompt = await handle_file_browser_callback(paths, "sf:workspace")

        assert prompt is None
        assert keyboard is not None
        assert "~/.controlmesh/workspace/" in text
        assert "skills/" in text
        assert "tools/" in text
        assert "CLAUDE.md" in text

    async def test_subfolder_has_back_button(self, paths: ControlMeshPaths) -> None:
    ...
    async def test_deep_navigation(self, paths: ControlMeshPaths) -> None:
        text, keyboard, _ = await handle_file_browser_callback(paths, "sf:workspace/tools")

        assert "~/.controlmesh/workspace/tools/" in text
        assert "cron_tools/" in text
        assert keyboard is not None

    async def test_deep_back_goes_to_parent(self, paths: ControlMeshPaths) -> None:
    ...
    async def test_excludes_pycache(self, paths: ControlMeshPaths) -> None:
        text, _, _ = await handle_file_browser_callback(paths, "sf:workspace")

        assert "__pycache__" not in text

    async def test_empty_directory(self, paths: ControlMeshPaths) -> None:
        text, keyboard, _ = await handle_file_browser_callback(paths, "sf:logs")

        assert "(empty)" in text
        assert keyboard is not None

    async def test_nonexistent_directory(self, paths: ControlMeshPaths) -> None:
...
class TestFileRequest:
    async def test_file_request_returns_prompt(self, paths: ControlMeshPaths) -> None:
    ...
    async def test_file_request_root(self, paths: ControlMeshPaths) -> None:
        _, _, prompt = await handle_file_browser_callback(paths, "sf!")

        assert prompt is not None
        assert str(paths.controlmesh_home.resolve()) in prompt
...
class TestPathSafety:
    async def test_parent_traversal_blocked(self, paths: ControlMeshPaths) -> None:
        text, _, _ = await handle_file_browser_callback(paths, "sf:..")

        assert "not found" in text.lower()

    async def test_double_parent_traversal_blocked(self, paths: ControlMeshPaths) -> None:
        text, _, _ = await handle_file_browser_callback(paths, "sf:workspace/../../etc")

        assert "not found" in text.lower()

    async def test_absolute_path_blocked(self, paths: ControlMeshPaths) -> None:
