...
class TestSendFile:
    ...
    async def test_image_sent_as_photo(self, tmp_path: Path) -> None:
        from controlmesh.messenger.telegram.sender import send_file
    ...
    async def test_unsupported_image_sent_as_document(self, tmp_path: Path) -> None:
        ...
        bot.send_photo = AsyncMock()
        bot.send_document = AsyncMock()
        with patch("controlmesh.messenger.telegram.sender.guess_mime", return_value="image/heic"):
            await send_file(bot, chat_id=1, path=img)
    ...
    async def test_supported_video_sent_as_video(self, tmp_path: Path) -> None:
        ...
        with patch("controlmesh.messenger.telegram.sender.guess_mime", return_value="video/mp4"):
    ...
    async def test_unsupported_video_sent_as_document(self, tmp_path: Path) -> None:
        ...
        bot.send_video = AsyncMock()
        bot.send_document = AsyncMock()
        with patch("controlmesh.messenger.telegram.sender.guess_mime", return_value="video/webm"):
            await send_file(bot, chat_id=1, path=video)
    ...
    async def test_supported_audio_sent_as_audio(self, tmp_path: Path) -> None:
        ...
        with patch("controlmesh.messenger.telegram.sender.guess_mime", return_value="audio/mpeg"):
    ...
    async def test_unsupported_audio_sent_as_document(self, tmp_path: Path) -> None:
        ...
        bot.send_audio = AsyncMock()
        bot.send_document = AsyncMock()
        with patch("controlmesh.messenger.telegram.sender.guess_mime", return_value="audio/wav"):
            await send_file(bot, chat_id=1, path=audio)
...
class TestExtractFilePaths:
    ...
    def test_single_file(self) -> None:
        from controlmesh.messenger.telegram.sender import extract_file_paths
...
class TestSendFilesFromText:
    ...
    async def test_sends_files_from_tags(self, tmp_path: Path) -> None:
        from controlmesh.messenger.telegram.sender import send_files_from_text
    ...
    async def test_windows_slash_drive_tag_normalized_before_send(self) -> None:
        ...
        with (
            patch("controlmesh.files.tags.is_windows", return_value=True),
            patch(
                "controlmesh.messenger.telegram.sender.send_file", new_callable=AsyncMock
            ) as mock_send_file,
        ):
