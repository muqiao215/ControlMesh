...
from controlmesh.config import DockerConfig
from controlmesh.workspace.paths import ControlMeshPaths


@pytest.fixture
def docker_paths(tmp_path: Path) -> ControlMeshPaths:
...
class TestDockerManager:
    ...
    async def test_mounts_full_controlmesh_home(
        ...
        """Verify run command mounts entire ~/.controlmesh, not just workspace."""
        from controlmesh.infra.docker import DockerManager
        ...
        # Full controlmesh_home mounted at /controlmesh
        assert f"{docker_paths.controlmesh_home}:/controlmesh" in run_str
        # Working dir is /controlmesh/workspace
        assert "-w /controlmesh/workspace" in run_str
        # CONTROLMESH_HOME env var set inside container
        assert "CONTROLMESH_HOME=/controlmesh" in run_str
    ...
    async def test_uid_mapping_on_linux(
        ...
        with (
            patch("shutil.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("controlmesh.infra.docker._needs_uid_mapping", return_value=True),
    ...
    async def test_no_uid_mapping_on_macos(
        ...
        with (
            patch("shutil.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("controlmesh.infra.docker._needs_uid_mapping", return_value=False),
        ):
            await mgr.setup()
    ...
    async def test_sub_agent_mounts_root_controlmesh_home(
        self, docker_config: DockerConfig, tmp_path: Path
    ) -> None:
        """Sub-agent container mounts ~/.controlmesh (root), not ~/.controlmesh/agents/test."""
        from controlmesh.infra.docker import DockerManager

        root_home = tmp_path / ".controlmesh"
        ...
        paths = ControlMeshPaths(
            controlmesh_home=agent_home, home_defaults=fw / "workspace", framework_root=fw
        )
        ...
        assert f"{root_home}:/controlmesh" in run_str
        assert f"{agent_home}:/controlmesh" not in run_str
