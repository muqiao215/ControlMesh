...
from controlmesh.infra.service_macos import (
    _LABEL,
...
class TestGeneratePlistData:
    def test_contains_binary_path(self) -> None:
        data = _generate_plist_data("/usr/local/bin/controlmesh")
        assert data["ProgramArguments"] == ["/usr/local/bin/controlmesh"]

    def test_has_label(self) -> None:
        data = _generate_plist_data("controlmesh")
        assert data["Label"] == "dev.controlmesh"
    ...
    def test_path_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("controlmesh.infra.service_macos.Path.home", return_value=tmp_path):
            data = _generate_plist_data("controlmesh")
    ...
    def test_generates_valid_plist(self) -> None:
        ...
        assert parsed["Label"] == "dev.controlmesh"
        assert parsed["RunAtLoad"] is True


class TestIsServiceInstalled:
    @patch("controlmesh.infra.service_macos._plist_path")
...
class TestIsServiceRunning:
    @patch("controlmesh.infra.service_macos.is_service_installed", return_value=False)
...
class TestInstallService:
    @patch("controlmesh.infra.service_macos._run_launchctl")
    @patch("controlmesh.infra.service_macos.is_service_installed", return_value=False)
    @patch("controlmesh.infra.service_macos.is_service_available", return_value=True)
    @patch(
        "controlmesh.infra.service_macos.find_controlmesh_binary", return_value="/usr/local/bin/controlmesh"
    )
    @patch("controlmesh.infra.service_macos._plist_path")
    @patch("controlmesh.infra.service_macos.resolve_paths")
    def test_install_success(
        ...
        plist_file = tmp_path / "dev.controlmesh.plist"
        ...
        assert plist_data["Label"] == "dev.controlmesh"

    @patch("controlmesh.infra.service_macos.is_service_available", return_value=False)
    def test_install_fails_without_launchctl(self, _avail: MagicMock) -> None:
        console = MagicMock()
        assert install_service(console) is False

    @patch("controlmesh.infra.service_macos.is_service_available", return_value=True)
    @patch("controlmesh.infra.service_macos.find_controlmesh_binary", return_value=None)
...
class TestStopService:
    @patch("controlmesh.infra.service_macos._run_launchctl")
    @patch("controlmesh.infra.service_macos.is_service_running", return_value=True)
    ...
    @patch("controlmesh.infra.service_macos.is_service_running", return_value=False)
...
class TestPrintServiceLogs:
    ...
    def test_shows_logs_from_file(
        ...
        logs_dir.mkdir()
        log_file = logs_dir / "controlmesh_2026-02-22.log"
