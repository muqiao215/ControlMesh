...
class TestDockerManagerMounts:
    ...
    async def test_single_mount_in_run_cmd(self, tmp_path: Path, docker_paths: ControlMeshPaths) -> None:
        proj = tmp_path / "myapp"
        proj.mkdir()
    ...
    async def test_multiple_mounts_in_run_cmd(
        self, tmp_path: Path, docker_paths: ControlMeshPaths
    ) -> None:
    ...
    async def test_empty_mounts_list(self, docker_paths: ControlMeshPaths) -> None:
...
class TestDockerMountCLI:
    ...
    def test_mount_adds_path(self, tmp_path: Path) -> None:
        from controlmesh.cli_commands.docker import docker_mount
        ...
        with patch("controlmesh.cli_commands.docker.resolve_paths", return_value=paths_mock):
    ...
    def test_unmount_removes_path(self, tmp_path: Path) -> None:
        from controlmesh.cli_commands.docker import docker_unmount
    ...
    def test_mounts_list_empty(self, tmp_path: Path) -> None:
        from controlmesh.cli_commands.docker import docker_list_mounts
