...
- [`config.example.json`](/root/.ductor/analysis/ductor-src/config.example.json) now ships a valid disabled gateway example. The previously enabled `ask-user-question` event no longer referenced a disabled gateway.
- [`config.example.json`](/root/.ductor/analysis/ductor-src/config.example.json) now leaves `api.token` empty so `ductor api enable` generates a real token instead of preserving the placeholder.

Evidence tests were added, without changing runtime contracts:

- [`tests/test_config.py`](/root/.ductor/analysis/ductor-src/tests/test_config.py) now validates the shipped example config against `AgentConfig`.
- [`tests/api/test_admin_catalog.py`](/root/.ductor/analysis/ductor-src/tests/api/test_admin_catalog.py) now covers empty-source catalogs, missing catalog reader (`503`), and practical concurrent read-only access.

## Deployment Evidence

Fresh verification artifacts were captured under [`task_artifacts/prod_hardening/`](/root/.ductor/analysis/ductor-src/task_artifacts/prod_hardening) plus the runbook [`task_artifacts/prod_hardening_runbook.md`](/root/.ductor/analysis/ductor-src/task_artifacts/prod_hardening_runbook.md).
...
1. `python3 -m venv task_artifacts/prod_hardening/.venv`
   ...
   Evidence: `install_editable_api_test.log` shows `Package 'ductor' requires a different Python: 3.10.12 not in '>=3.11'`.
...
4. `. task_artifacts/prod_hardening/.venv312/bin/activate && export DUCTOR_HOME=$PWD/task_artifacts/prod_hardening/fresh_home_clean2 && python - <<PY ... load_config()`
...
5. `. task_artifacts/prod_hardening/.venv312/bin/activate && export DUCTOR_HOME=$PWD/task_artifacts/prod_hardening/fresh_home_clean2 && ductor api enable`
...
- Empty derived sources return empty catalogs.
  Evidence: new test in [`tests/api/test_admin_catalog.py`](/root/.ductor/analysis/ductor-src/tests/api/test_admin_catalog.py).
...
- Missing team snapshot returns structured `not_found`.
  Evidence: existing [`tests/team/test_api.py`](/root/.ductor/analysis/ductor-src/tests/team/test_api.py) and `recovery_smoke.log`.
- Snapshot refresh rebuilds from canonical team state.
  Evidence: existing [`tests/team/test_api.py`](/root/.ductor/analysis/ductor-src/tests/team/test_api.py), [`tests/team/test_snapshot_recovery.py`](/root/.ductor/analysis/ductor-src/tests/team/test_snapshot_recovery.py), and `recovery_smoke.log`.
...
- Log path verified: `$DUCTOR_HOME/logs/agent.log`
...
- API verification commands and expected responses are captured in [`task_artifacts/prod_hardening_runbook.md`](/root/.ductor/analysis/ductor-src/task_artifacts/prod_hardening_runbook.md).
...
- `ductor api enable` now produces a usable persisted token from the shipped example config.
...
- Full production deployment evidence is still incomplete because this cut did not run the entire supervised `ductor` process with real transport credentials and at least one authenticated provider CLI.
- Installed-service behavior (`ductor service install/start/logs`) was not exercised on a live service manager in this cut.
