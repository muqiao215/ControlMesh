...
def _device_flow_auth_path(ductor_home: str | Path) -> Path:
    return Path(ductor_home).expanduser() / "feishu_store" / "auth" / "device_flow.json"
...
def persist_device_flow_auth(  # noqa: PLR0913
    *,
    ...
    path = _device_flow_auth_path(ductor_home)
...
def load_device_flow_auth(
    *,
    ...
    data = load_json(_device_flow_auth_path(ductor_home))
...
def clear_device_flow_auth(*, ductor_home: str | Path, app_id: str) -> None:
    path = _device_flow_auth_path(ductor_home)
    stored = load_device_flow_auth(ductor_home=ductor_home, app_id=app_id)
    if stored is None:
        return
    with contextlib.suppress(OSError):
        path.unlink()
...
def get_feishu_auth_status(*, config: AgentConfig, now_ms: int) -> FeishuAuthStatus:
    stored = load_device_flow_auth(ductor_home=config.ductor_home, app_id=config.feishu.app_id)
