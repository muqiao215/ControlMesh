"""File bootstrap helpers for Ductor memory-v2."""
...
from ductor_bot.memory.models import DreamingSweepState
from ductor_bot.workspace.paths import DuctorPaths

_AUTHORITY_TEMPLATE = """# Ductor Memory v2
...
_DREAM_DIARY_TEMPLATE = """# Ductor Dream Diary
...
def initialize_memory_v2(paths: DuctorPaths) -> None:
    ...
    if not paths.dreaming_sweep_state_path.exists():
        from ductor_bot.memory.dreaming import save_sweep_state

        save_sweep_state(paths, DreamingSweepState())
    if not paths.dreaming_checkpoints_path.exists():
        from ductor_bot.memory.dreaming import save_checkpoints
