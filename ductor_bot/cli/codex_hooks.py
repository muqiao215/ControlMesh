"""Codex native-hook capability mapping for Ductor.

This module does not execute hooks. It documents and exposes the current
capability matrix so Ductor can reason explicitly about which lifecycle
...
from ductor_bot.config import CodexHooksConfig
...
class CodexHookCapability:
    """One Codex lifecycle surface and how Ductor currently expects to handle it."""
...
class CodexHookStatus:
    ...
    def effective_mode(self) -> Literal["native_preferred", "runtime_fallback"]:
        """Return how Ductor should treat this project today."""
        if self.ductor_enabled and self.prefer_native and self.native_hooks_configured:
...
DEFAULT_CODEX_HOOK_CAPABILITIES: tuple[CodexHookCapability, ...] = (
    ...
    CodexHookCapability(
        ...
        native_source="Stop",
        fallback_owner="orchestrator/hooks.py",
        notes="Stop continuation exists natively, but Ductor-specific continuation policy still needs orchestrator ownership.",
    ),
    CodexHookCapability(
        event="ask-user-question",
        mode="runtime_fallback",
        fallback_owner="tasks/hub.py",
        notes="Ductor already has a parent-question path through TaskHub rather than a Codex-native surface.",
    ),
    CodexHookCapability(
    ...
    CodexHookCapability(
        ...
        mode="runtime_fallback",
        fallback_owner="orchestrator/hooks.py",
        notes="Current Codex native tool interception scope is narrower than Ductor's overall tool model.",
    ),
    CodexHookCapability(
        event="session-end",
        mode="runtime_fallback",
        fallback_owner="bus/bus.py",
        notes="Best modeled through Ductor runtime or gateway delivery for now.",
    ),
    CodexHookCapability(
        event="session-idle",
        mode="runtime_fallback",
        fallback_owner="bus/bus.py",
        notes="Idle detection remains runtime-owned in Ductor today.",
    ),
    CodexHookCapability(
        event="subagent-stop",
        mode="not_supported",
        notes="No Codex-native equivalent exists today for Ductor's multi-agent lifecycle edge.",
    ),
)
...
def inspect_codex_hook_status(
    ...
    return CodexHookStatus(
        paths=paths,
        ductor_enabled=settings.enabled,
