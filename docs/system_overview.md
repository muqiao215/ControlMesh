...
Fastest end-to-end mental model for controlmesh.
...
- optional sub-agent stacks from `~/.controlmesh/agents.json`
...
Sub-agent home: `~/.controlmesh/agents/<name>/` with its own config/workspace/session files.

## 8) Where to read code first

1. `controlmesh/__main__.py` (entrypoint + config/load/run)
2. `controlmesh/cli_commands/` (actual CLI subcommand logic)
3. `controlmesh/multiagent/supervisor.py` (always-on runtime wrapper)
4. `controlmesh/messenger/telegram/app.py` + `messenger/telegram/startup.py` (Telegram), `controlmesh/messenger/matrix/bot.py` (Matrix)
5. `controlmesh/orchestrator/core.py` + `orchestrator/lifecycle.py`
6. `controlmesh/bus/*` (unified delivery/injection)
7. `controlmesh/tasks/hub.py` + `tasks/registry.py`
8. `controlmesh/cli/service.py` and provider wrappers
...
- Command menus: `/cm` opens Native Commands for the current CLI, `/back` returns to ControlMesh commands, `/cm <controlmesh-command>` runs one ControlMesh command from the native menu
...
- `controlmesh`
- `controlmesh status|stop|restart|upgrade|uninstall|onboarding|reset|help`
- `controlmesh service ...`
- `controlmesh docker ...` (includes `extras`, `extras-add`, `extras-remove` for optional AI/ML packages)
- `controlmesh api ...`
- `controlmesh agents ...` (`add` currently scaffolds Telegram sub-agents; Matrix sub-agents are added via `agents.json` or tool scripts)
- `controlmesh install <extra>` (`matrix`, `api`)
...
- `controlmesh service logs` follows `journalctl` on Linux and tails file logs on macOS/Windows
