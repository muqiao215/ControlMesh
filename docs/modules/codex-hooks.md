...
This module documents Ductor's current view of Codex-native lifecycle support
and inspects whether a given repo is actually configured for native Codex
hooks.

Source:

- implementation: `ductor_bot/cli/codex_hooks.py`

## Purpose

Ductor already supports the Codex CLI as a provider, but provider support does
...
- which still belong to Ductor runtime fallback
...
- `native_hooks_configured`: native hooks are available and `.codex/hooks.json`
  also exists
- `effective_mode`: whether Ductor should prefer native hooks right now or stay
