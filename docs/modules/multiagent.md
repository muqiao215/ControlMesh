...
Multi-agent runtime: run multiple independent controlmesh agents in one process.
...
- workspace and files under `~/.controlmesh/agents/<name>/`
...
- central log file (`~/.controlmesh/logs/agent.log`)
- shared knowledge source (`~/.controlmesh/SHAREDMEMORY.md`)
...
- `controlmesh agents`
- `controlmesh agents list`
- `controlmesh agents add <name>`
- `controlmesh agents remove <name>`

`controlmesh agents list` fetches live health from internal API when main bot is running.

Important CLI nuance:

- `controlmesh agents add <name>` currently prompts for Telegram token/user/group data only.
