...
- `controlmesh/config_reload.py`: `ConfigReloader`, config diff/classification helpers

## Purpose

Avoid unnecessary restarts when safe `config.json` fields change.

`ConfigReloader` polls `~/.controlmesh/config/config.json` every 5 seconds, validates with `AgentConfig`, diffs top-level schema fields, and:
...
- environment/core: `controlmesh_home`, `log_level`, `gemini_api_key`, `update_check`
