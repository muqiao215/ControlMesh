...
controlmesh supports multiple messaging transports. The `transport` config
...
### `controlmesh` entry (`controlmesh/__main__.py`)
...
1. resolve paths and set `CONTROLMESH_HOME` for main agent
...
- shared registry: `~/.controlmesh/tasks.json`
...
- task tools receive `CONTROLMESH_CHAT_ID` and optional `CONTROLMESH_TOPIC_ID`
...
Source: `controlmesh/_home_defaults/`.
