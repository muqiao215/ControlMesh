...
- Current ControlMesh/Ductor session persistence is metadata-oriented, not transcript-oriented.
- `ductor_bot/session/manager.py` stores `provider_sessions`, `session_id`, `message_count`, `total_cost_usd`, and `total_tokens`; it does not model visible message transcript turns.
- `ductor_bot/session/named.py` stores named-session metadata such as `prompt_preview`, `status`, `message_count`, and `last_prompt`; it is a background-session registry, not a visible chat transcript store.
...
## Resources
- `/root/.ductor/analysis/ductor-src/docs/modules/session.md`
- `/root/.ductor/analysis/ductor-src/ductor_bot/session/manager.py`
- `/root/.ductor/analysis/ductor-src/ductor_bot/session/named.py`
- `/root/.ductor/workspace/repos/cc-connect/docs/usage.md`
- `/root/.ductor/workspace/repos/cc-connect/config.example.toml`
- `/root/.ductor/workspace/repos/cc-connect/CHANGELOG.md`
- `/root/.ductor/workspace/repos/cc-connect/core/engine.go`
- `/root/.ductor/workspace/repos/cc-connect/cmd/cc-connect/sessions.go`
- `/root/.ductor/workspace/repos/my-programming-world/Wiki/自动化开发范式与智能体协作.md`
- `/root/.ductor/workspace/repos/my-programming-world/学语与公众号/公众号/草稿/Superpowers与Plan-with-Files_把AI从聊天搭子变成长期项目搭子.md`
- `/root/.ductor/workspace/repos/my-programming-world/编程/浏览器自动化大结局：远程登录加持久化 profile 才是正解.md`
- `/root/.ductor/workspace/repos/my-programming-world/编程/开发指南/PLC/PLC 完整项目运行清单.md`
...
- Normal foreground message handling in `ductor_bot/orchestrator/flows.py`
