...
from controlmesh.messenger.commands import classify_command
from controlmesh.messenger.matrix.buttons import ButtonTracker
from controlmesh.messenger.matrix.credentials import login_or_restore
...
def _expand_marker(controlmesh_home: str) -> Path:
    """Return the restart-marker path (sync, no I/O)."""
    return Path(controlmesh_home).expanduser() / "restart-requested"
...
class MatrixBot:
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'controlmesh[matrix]'"
            ) from None

        self._config = config
        self._agent_name = agent_name
        mx = config.matrix
        self._store_path = Path(config.controlmesh_home).expanduser() / mx.store_path
        ...
        from controlmesh.messenger.matrix.transport import MatrixTransport
    ...
    async def run(self) -> int:
        ...
        from controlmesh.messenger.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from controlmesh.messenger.matrix.media import resolve_matrix_media
    ...
    async def _cmd_showfiles(
        ...
        from controlmesh.messenger.matrix.file_browser import format_file_listing
    ...
    async def _send_selector_response(
        ...
        from controlmesh.messenger.matrix.buttons import REACTION_DIGITS
    ...
    async def _handle_upgrade_callback(self, room_id: str, data: str) -> None:
        ...
        if data.startswith("upg:cl:"):
            ...
            if not version:
                return
            from controlmesh.infra.version import fetch_changelog
    ...
    async def _handle_ns_callback(self, room_id: str, key: SessionKey, data: str) -> None:
        ...
        from controlmesh.messenger.telegram.callbacks import parse_ns_callback
