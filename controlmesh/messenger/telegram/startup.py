...
async def _handle_restart_sentinel(bot: TelegramBot) -> dict[str, object] | None:
    ...
    sentinel_path = bot._orch.paths.controlmesh_home / "restart-sentinel.json"
...
async def _handle_recovery(bot: TelegramBot, sentinel: dict[str, object] | None) -> None:
    ...
    from controlmesh.infra.startup_state import detect_startup_kind, save_startup_state
    from controlmesh.text.response_format import startup_notification_text
    ...
    from controlmesh.infra.recovery import RecoveryPlanner
    from controlmesh.text.response_format import recovery_notification_text
...
async def _run_primary_startup(bot: TelegramBot) -> None:
    ...
    from controlmesh.messenger.telegram.chat_tracker import ChatTracker
