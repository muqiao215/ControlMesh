...
def _docker_skill_resync(paths: ControlMeshPaths) -> None:
...
async def create_orchestrator(
    ...
    *,
    ...
    if agent_name == "main":
        os.environ["CONTROLMESH_HOME"] = str(paths.controlmesh_home)
...
async def start_api_server(
    ...
    try:
        ...
        logger.warning(
            "API server enabled but PyNaCl is not installed. Install with: pip install controlmesh[api]"
        )
        return
...
async def shutdown(orch: Orchestrator) -> None:
    ...
    await asyncio.to_thread(cleanup_controlmesh_links, orch._paths)
