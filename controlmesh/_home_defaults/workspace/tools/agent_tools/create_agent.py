...
def _agents_path() -> Path:
    ...
    Sub-agents have CONTROLMESH_HOME = ~/.controlmesh/agents/<name>/, so we navigate
    up to the main home. Main agent's CONTROLMESH_HOME points directly to ~/.controlmesh/.
    """
    import os
...
def _main_home() -> Path:
    """Resolve the main agent's CONTROLMESH_HOME."""
    import os

    home = Path(os.environ.get("CONTROLMESH_HOME", str(Path.home() / ".controlmesh")))
    if home.name != ".controlmesh" and (home.parent.parent / "config").is_dir():
