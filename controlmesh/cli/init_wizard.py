...
from controlmesh.cli.auth import AuthStatus, check_claude_auth, check_codex_auth, check_gemini_auth
from controlmesh.config import DEFAULT_EMPTY_GEMINI_API_KEY, AgentConfig, deep_merge_config
...
def _build_extras_table(console: Console) -> None:
    ...
    from controlmesh.infra.docker_extras import DOCKER_EXTRAS_BY_ID, extras_for_display
...
def _ask_docker_extras(console: Console) -> list[str]:
    ...
    from controlmesh.infra.docker_extras import (
        DOCKER_EXTRAS_BY_ID,
        extras_for_display,
        resolve_extras,
    )
...
def _offer_service_install(console: Console) -> bool:
    """Ask whether to install controlmesh as a background service."""
    from controlmesh.infra.service import is_service_available
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            ...
            + "\n\n"
            + t_rich("wizard.complete.home", path=paths.controlmesh_home)
            + "\n"
    ...
    if run_as_service:
        from controlmesh.infra.service import install_service
...
def run_smart_reset(controlmesh_home: Path) -> None:
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                docker_container = str(docker.get("container_name", "controlmesh-sandbox"))
                docker_image = str(docker.get("image_name", "controlmesh-sandbox"))
        except (json.JSONDecodeError, OSError):
            pass
    ...
    console.print(
        Panel(
            t_rich("wizard.reset.body", home=controlmesh_home),
    ...
    if controlmesh_home.exists():
        console.print(t_rich("wizard.reset.confirm.warning", home=controlmesh_home) + "\n")
    else:
