...
from controlmesh.cli.auth import gemini_api_key_mode_selected
...
from controlmesh.cli.gemini_events import (
    extract_result_text,
    extract_text,
    parse_gemini_stream_line,
)
from controlmesh.cli.gemini_utils import (
...
if TYPE_CHECKING:
    from controlmesh.cli.process_registry import ProcessRegistry, TrackedProcess
...
# Must match ``_CONTROLMESH_MOUNT`` in ``controlmesh.infra.docker``.
_CONTAINER_CONTROLMESH = "/controlmesh"
...
class GeminiCLI(BaseCLI):
    ...
    def _inject_config_gemini_api_key(self, env: dict[str, str]) -> None:
        """Inject GEMINI_API_KEY from controlmesh config when API-key auth mode is active."""
        ...
        logger.debug("Injected GEMINI_API_KEY from controlmesh config for Gemini API key mode")
    ...
    def _create_system_prompt_path(self) -> str | None:
        ...
        In Docker mode the file is written to ``~/.controlmesh/tmp/`` which is
        ...
        if self._config.docker_container:
            tmp_dir = resolve_paths().controlmesh_home / "tmp"
    ...
    def _host_to_container_path(host_path: str) -> str | None:
        """Translate a host path under ``~/.controlmesh/`` to its container mount."""
        prefix = str(resolve_paths().controlmesh_home)
        if host_path.startswith(prefix):
            return _CONTAINER_CONTROLMESH + host_path[len(prefix) :].replace("\\", "/")
        return None
