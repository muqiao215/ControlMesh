...
def _to_container_path(host_path: Path, main_home: Path) -> str:
    ...
    The Docker container mounts the root controlmesh home at ``/controlmesh``.
    """
    rel = host_path.relative_to(main_home)
    if str(rel) == ".":
        return _CONTAINER_CONTROLMESH_MOUNT
    return f"{_CONTAINER_CONTROLMESH_MOUNT}/{rel.as_posix()}"
...
def docker_wrap(
    ...
    *extra_env* vars are injected as ``-e`` flags into ``docker exec``
    ...
    if config.docker_container:
        ...
        env_flags: list[str] = [
            "-e",
            f"CONTROLMESH_CHAT_ID={config.chat_id}",
            "-e",
            f"CONTROLMESH_TRANSPORT={config.transport}",
            "-e",
            f"CONTROLMESH_AGENT_NAME={config.agent_name}",
            "-e",
            f"CONTROLMESH_INTERAGENT_PORT={config.interagent_port}",
            "-e",
            f"CONTROLMESH_HOME={container_home}",
            "-e",
            f"CONTROLMESH_SHARED_MEMORY_PATH={container_shared}",
            "-e",
            "CONTROLMESH_INTERAGENT_HOST=host.docker.internal",
        ]
        if config.topic_id:
            env_flags += ["-e", f"CONTROLMESH_TOPIC_ID={config.topic_id}"]
        if extra_env:
