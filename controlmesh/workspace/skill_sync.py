"""Cross-platform skill directory sync between controlmesh workspace and CLI tools.

Provides multi-way symlink synchronization so skills installed via Claude Code,
Codex CLI, Gemini CLI, or the controlmesh workspace are visible to all agents.

Includes bundled-skill linking (package → workspace), sync-time external-symlink
protection, and cleanup of controlmesh-created links on shutdown.

When Docker sandboxing is active, symlinks are replaced with directory copies
(marked with ``.controlmesh_managed``) because absolute host paths do not resolve
...
def _resolve_canonical(
    ...
    *registries: dict[str, Path],
    ...
    Priority follows argument order (typically controlmesh > claude > codex > gemini).
...
def _is_managed_copy(path: Path) -> bool:
    """Return ``True`` if *path* is a controlmesh-managed copy (has marker file)."""
...
def _ensure_copy(dest: Path, source: Path) -> bool:
    """Copy *source* directory to *dest* with a ``.controlmesh_managed`` marker.
...
def sync_skills(paths: ControlMeshPaths, *, docker_active: bool = False) -> None:
    """Multi-way skill directory sync: controlmesh workspace <-> CLI skill dirs.

    Syncs between controlmesh workspace, ~/.claude/skills, ~/.codex/skills,
    and ~/.gemini/skills.
    ...
    # Priority order: controlmesh > claude > codex > gemini
    priority = ("controlmesh", "claude", "codex", "gemini")
...
def sync_bundled_skills(paths: ControlMeshPaths, *, docker_active: bool = False) -> None:
    """Sync bundled skills from the package into the controlmesh workspace.

    Creates symlinks (or copies when *docker_active*) from
    ``~/.controlmesh/workspace/skills/<name>`` to the package's
    ``_home_defaults/workspace/skills/<name>`` so bundled skills
    stay up-to-date with the installed controlmesh version.
...
def cleanup_controlmesh_links(paths: ControlMeshPaths) -> int:
    """Remove symlinks created by controlmesh in CLI skill directories.

    Only removes symlinks whose resolved target is under the controlmesh workspace
    ...
    for cli_dir in _cli_skill_dirs().values():
        ...
        for entry in cli_dir.iterdir():
            ...
            if any(_is_under(resolved, root) for root in managed_roots):
                entry.unlink()
                removed += 1
                logger.info("Removed controlmesh skill link: %s", entry)

    if removed:
        logger.info("Cleaned up %d controlmesh skill link(s) from CLI directories", removed)
    return removed
