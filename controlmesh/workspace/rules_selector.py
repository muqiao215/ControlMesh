...
class RulesSelector:
    ...
    Deployed naming in ~/.controlmesh/:
    ...
    def deploy_rules(self) -> None:
        """Auto-discover and deploy all rule files to ~/.controlmesh/.

        Scans _home_defaults/ for directories with RULES templates, selects
        the best variant for current auth state, and deploys to ~/.controlmesh/
        ...
        for template_dir in template_dirs:
            ...
            dst_dir = self._paths.controlmesh_home / rel_path
    ...
    def _remove_files_by_name(self, filename: str) -> int:
        """Remove all files with given name in ~/.controlmesh/.
        ...
        """
        cron_tasks_path = self._paths.controlmesh_home / "workspace" / "cron_tasks"
        removed_count = 0
        for file_path in self._paths.controlmesh_home.rglob(filename):
            ...
            try:
                ...
                removed_count += 1
                logger.debug(
                    "Removed stale file: %s", file_path.relative_to(self._paths.controlmesh_home)
                )
            except OSError:
