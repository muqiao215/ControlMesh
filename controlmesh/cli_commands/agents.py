"""Agent management CLI subcommands (``controlmesh agents ...``)."""
...
def _parse_agents_subcommand(args: list[str]) -> tuple[str | None, list[str]]:
    ...
    for a in args:
        ...
        if found and sub is not None:
            rest.append(a)
    if found and sub is None:
        # bare "controlmesh agents" → default to list
        return "list", []
    return sub, rest
...
def print_agents_help() -> None:
    ...
    table.add_column()
    table.add_row("controlmesh agents", "List all sub-agents and their config")
    table.add_row("controlmesh agents list", "List all sub-agents and their config")
    table.add_row("controlmesh agents add <name>", "Add a new sub-agent (interactive)")
    table.add_row("controlmesh agents remove <name>", "Remove a sub-agent")
    _console.print(
...
def load_agents_registry(paths: ControlMeshPaths) -> list[dict[str, object]]:
    """Load sub-agent definitions from agents.json (raw dicts)."""
    agents_path = paths.controlmesh_home / "agents.json"
...
def fetch_live_health() -> dict[str, dict[str, object]]:
    ...
    from controlmesh.multiagent.internal_api import _DEFAULT_PORT
...
def validate_agent_name(name: str | None, agents: list[dict[str, object]]) -> str | None:
    """Validate an agent name for ``controlmesh agents add``. Returns clean name or None on error."""
    if not name:
...
def agents_remove(rest: list[str]) -> None:
    ...
    _console.print(t_rich("agents.remove.data_hint", path=paths.controlmesh_home / "agents" / name))


def cmd_agents(args: list[str]) -> None:
    """Handle 'controlmesh agents [subcommand]'."""
