"""Docker management CLI subcommands (``controlmesh docker ...``)."""
...
def _parse_docker_mount_arg(args: list[str]) -> str | None:
    ...
    Expects the form: ``controlmesh docker mount <path>`` where *args* is
    ``sys.argv[1:]`` (no ``controlmesh`` prefix).  Non-flag positionals are
...
def print_docker_help() -> None:
    ...
    table.add_column()
    table.add_row("controlmesh docker rebuild", "Remove container & image, rebuild on next start")
    table.add_row("controlmesh docker enable", "Enable Docker sandboxing")
    table.add_row("controlmesh docker disable", "Disable Docker sandboxing")
    table.add_row("controlmesh docker mount <path>", "Mount a host directory into the sandbox")
    table.add_row("controlmesh docker unmount <path>", "Remove a mounted directory")
    table.add_row("controlmesh docker mounts", "List all mounted directories")
    table.add_row("controlmesh docker extras", "List available and installed extras")
    table.add_row("controlmesh docker extras-add <id>", "Add an extra package")
    table.add_row("controlmesh docker extras-remove <id>", "Remove an extra package")
    _console.print(
...
def docker_rebuild() -> None:
    ...
    result = docker_read_config()
    container = "controlmesh-sandbox"
    image = "controlmesh-sandbox"
...
def docker_mount(args: list[str]) -> None:
    ...
    from controlmesh.infra.docker import resolve_mount_target
...
def docker_extras_list() -> None:
    ...
    from controlmesh.infra.docker_extras import extras_for_display
...
def docker_extras_add(args: list[str]) -> None:
    ...
    from controlmesh.infra.docker_extras import DOCKER_EXTRAS_BY_ID
...
def docker_extras_remove(args: list[str]) -> None:
    ...
    from controlmesh.infra.docker_extras import DOCKER_EXTRAS, DOCKER_EXTRAS_BY_ID
...
def cmd_docker(args: list[str]) -> None:
    """Handle 'controlmesh docker <subcommand>'."""
