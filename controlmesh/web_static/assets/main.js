...
// ../../packages/controlmesh-protocol/src/validators/schemas.ts
var controlmeshSchemas = {
  ...
  "team-director-decision.schema.json": {
    description: "Typed director control decision for director_worker loops.",
    properties: {
      schema_version: {
        const: 1
      },
    ...
    additionalProperties: false,
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: "https://controlmesh.dev/schemas/v1/team-director-decision.schema.json",
    title: "TeamDirectorDecision",
    allOf: [
      {
        if: {
          properties: {
  ...
  "team-judge-decision.schema.json": {
    description: "Typed judge control decision for debate_judge rounds.",
    properties: {
      schema_version: {
        const: 1
      },
    ...
    additionalProperties: false,
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: "https://controlmesh.dev/schemas/v1/team-judge-decision.schema.json",
    title: "TeamJudgeDecision",
    allOf: [
      {
        if: {
          properties: {
...
var ajv = new import__2020.default({ allErrors: true, strict: false });
...
function assertProtocolSchema(schemaName, value) {
  const validate = validators.get(schemaName);
  if (!validate || !validate(value)) {
    throw new ProtocolValidationError(schemaName, validate?.errors ? [...validate.errors] : []);
  }
}
// ../../packages/controlmesh-runtime-facade/src/http.ts
class ControlMeshRequestError extends Error {
  status;
  envelope;
...
// ../../packages/controlmesh-runtime-facade/src/sse.ts
...
// ../../packages/controlmesh-sdk/src/errors.ts
class ControlMeshError extends Error {
  code;
  status;
  details;
  retryable;
  ...
  constructor(envelope, options = {}) {
    ...
    this.correlationId = typeof envelope.correlation_id === "string" ? envelope.correlation_id : "";
  }
}

// ../../packages/controlmesh-sdk/src/client.ts
class ControlMeshClient {
  baseUrl;
  token;
  fetchImpl;
  constructor(options = {}) {
  ...
  listTasks(query = {}) {
    ...
    return this.requestValidatedListEnvelope(`/api/v1/tasks${params.size ? `?${params}` : ""}`, "task.schema.json");
  }
  getTask(taskId) {
    return this.requestValidated(`/api/v1/tasks/${encodeURIComponent(taskId)}`, "task.schema.json");
  }
  getTaskEvents(taskId) {
    return this.requestValidatedList(`/api/v1/tasks/${encodeURIComponent(taskId)}/events`, "task-event.schema.json");
  }
  async* subscribeTaskEvents(taskId) {
    const events = streamSse(`${this.baseUrl}/api/v1/tasks/${encodeURIComponent(taskId)}/events/stream`, { token: this.token, fetch: this.fetchImpl });
    for await (const event of events) {
      assertProtocolSchema("task-event.schema.json", event);
      yield event;
    }
  }
  listArtifacts(taskId) {
    return this.requestValidatedList(`/api/v1/tasks/${encodeURIComponent(taskId)}/artifacts`, "artifact.schema.json");
  }
  downloadArtifact(taskId, relativePath) {
    const params = new URLSearchParams({ relative_path: relativePath });
    return this.requestRaw(`/api/v1/tasks/${encodeURIComponent(taskId)}/artifacts/content?${params}`);
  }
  async getProviderStatus(provider) {
    const body = await this.requestValidatedListEnvelope("/api/v1/providers", "provider-capability.schema.json");
  ...
  async listTopologies() {
    const body = await this.requestValidatedListEnvelope("/api/v1/topologies", "topology.schema.json");
    return body.items;
  }
  async requestValidated(path, schemaName, options = {}) {
    const value = await this.request(path, options);
    assertProtocolSchema(schemaName, value);
    return value;
  }
  async requestValidatedList(path, schemaName, options = {}) {
    const value = await this.request(path, options);
    if (!Array.isArray(value)) {
      throw new ProtocolValidationError(schemaName);
    }
    for (const item of value) {
      assertProtocolSchema(schemaName, item);
    }
    return value;
  }
  async requestValidatedListEnvelope(path, schemaName) {
    const value = await this.request(path);
    if (typeof value !== "object" || value === null || !("items" in value) || !Array.isArray(value.items) || "total" in value && typeof value.total !== "number" || "limit" in value && typeof value.limit !== "number") {
      throw new ProtocolValidationError(schemaName);
    }
  ...
  request(path, options = {}) {
    return this.wrapErrors(requestJson(`${this.baseUrl}${path}`, {
  ...
  async wrapErrors(promise) {
    try {
      return await promise;
    } catch (error) {
      if (error instanceof ControlMeshRequestError) {
        assertProtocolSchema("error.schema.json", error.envelope);
...
var defaultBaseUrl = window.location.pathname.startsWith("/dashboard") ? window.location.origin : "http://127.0.0.1:8741";
var state = {
...
function wireEvents() {
  ...
  document.querySelectorAll("[data-artifact-path]").forEach((button) => {
    button.addEventListener("click", () => {
      downloadArtifact(button);
    });
  });
}
async function downloadArtifact(button) {
  ...
  try {
    const content = await client2().downloadArtifact(state.selectedTaskId, relativePath);
...
async function loadTaskDetail(shouldRender = true) {
