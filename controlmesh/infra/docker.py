...
class DockerManager:
    ...
    def __init__(self, config: DockerConfig, paths: ControlMeshPaths) -> None:
        self._config = config
        self._paths = paths
    ...
    async def _start_container(self, name: str, image: str) -> bool:
        # Always mount the root controlmesh home, even when called from a sub-agent.
        ...
        # like /controlmesh/agents/<name>/workspace.
        controlmesh_home = self._paths.controlmesh_home
        if controlmesh_home.parent.name == "agents":
            controlmesh_home = controlmesh_home.parent.parent
        ...
        cmd: list[str] = [
            ...
            "-w",
            _CONTAINER_WS,
            # Mount the ENTIRE ~/.controlmesh so the CLI sees all framework files.
            "-v",
            f"{controlmesh_home}:{_CONTROLMESH_MOUNT}",
            "-e",
            f"CONTROLMESH_HOME={_CONTROLMESH_MOUNT}",
    ...
    def _env_secret_flags(self) -> list[str]:
        """Return ``-e`` flags for user secrets from ``~/.controlmesh/.env``."""
