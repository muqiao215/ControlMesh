"""Platform-dispatching service management for controlmesh.
...
if sys.platform == "win32":
    from controlmesh.infra import service_windows as _backend
elif sys.platform == "darwin":
    from controlmesh.infra import service_macos as _backend
else:
    from controlmesh.infra import service_linux as _backend
...
def is_service_installed() -> bool:
    """Check if the controlmesh service is installed."""
    return _backend.is_service_installed()


def is_service_running() -> bool:
    """Check if the controlmesh service is currently running."""
    return _backend.is_service_running()


def install_service(console: Console | None = None) -> bool:
    """Install and start the controlmesh background service. Returns True on success."""
    return _backend.install_service(console)


def uninstall_service(console: Console | None = None) -> bool:
    """Stop and remove the controlmesh background service."""
