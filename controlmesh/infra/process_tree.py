...
def kill_all_controlmesh_processes() -> int:
    """Find and force-kill remaining ``controlmesh`` processes system-wide.

    On Windows this uses two strategies:
    1. ``tasklist`` scan for ``controlmesh.exe`` processes
    2. PowerShell scan for ``python.exe``/``pythonw.exe`` running from the
       pipx venv directory (covers ``pythonw.exe -m controlmesh``)
    ...
    current = os.getpid()
    killed = _kill_controlmesh_exe_windows(current)
    killed += _kill_venv_python_windows(current)
    return killed


def _kill_controlmesh_exe_windows(current_pid: int) -> int:
    """Kill processes whose image name is ``controlmesh.exe``."""
    try:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) < 2:
            continue
        name = parts[0].lower()
        if name not in ("controlmesh.exe", "controlmesh"):
            continue
        with contextlib.suppress(ValueError):
            pid = int(parts[1])
            if pid == current_pid or pid <= 0:
                continue
            logger.info("Killing controlmesh process: pid=%d name=%s", pid, name)
            _run_taskkill(pid, force=True)
            killed += 1
    return killed
...
def _kill_venv_python_windows(current_pid: int) -> int:
    ...
    When controlmesh is installed via ``pipx``, the bot runs as
    ``pythonw.exe -m controlmesh`` inside ``~\pipx\venvs\controlmesh\``.
        ...
        result = subprocess.run(
            [
                ...
                (
                    ...
                    " | Where-Object { $_.Path -like '*pipx*controlmesh*' }"
