...
export function parseRuntimeSessionKey(raw: string): RuntimeSessionKey {
  ...
  requireThat(/^[A-Za-z0-9_-]{1,64}$/.test(result.transport), "invalid_session_transport");
  return result;
}
...
export function runtimeSessionKeyFromAddress(transport: string, chat: string | number | bigint, topic: string | number | bigint | null = null): string {
  const encode = (value: string | number | bigint) => {
    if (typeof value === "string") return `s:${quote(value)}`;
    requireThat(typeof value === "bigint" || Number.isSafeInteger(value), "invalid_session_integer");
    return `i:${value.toString()}`;
  };
  return runtimeSessionStorageKey(`v2:${transport}:${encode(chat)}${topic === null ? "" : `:${encode(topic)}`}`);
}
