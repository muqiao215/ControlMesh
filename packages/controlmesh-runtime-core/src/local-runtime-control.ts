  ...
  async handle(request: unknown): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const fields: Record<string, readonly string[]> = {
        ...
        host_log: ["task_id", "effect_id", "after", "limit"],
      ...
      switch (request.op) {
        ...
        case "host_log": identifier(request.task_id); result = this.runtime.hostLog(request.task_id, request.effect_id as string | undefined, request.after as number | undefined, request.limit as number | undefined); break;
        ...
        case "session_events": requireThat(typeof request.session_key === "string", "invalid_session_key"); result = this.runtime.sessionEvents(request.session_key, request.limit as number | undefined, request.before as number | undefined); break;
