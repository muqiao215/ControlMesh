  ...
  constructor(path: string, private readonly clock: () => number = Date.now) {
    ...
    try {
      ...
      this.transaction(() => {
        ...
        if (version < 26) {
          this.sql.exec(`
            CREATE TABLE topology_native_inputs (
              run_id TEXT PRIMARY KEY, payload TEXT NOT NULL, digest TEXT NOT NULL,
        ...
        if (version < 40) {
          this.sql.exec(`CREATE TABLE delivery_media (
            id TEXT PRIMARY KEY, principal TEXT NOT NULL, task_id TEXT NOT NULL REFERENCES tasks(task_id),
            task_revision INTEGER NOT NULL, event_seq INTEGER NOT NULL REFERENCES events(seq),
            binding_digest TEXT NOT NULL, metadata TEXT NOT NULL, source TEXT NOT NULL,
            content BLOB NOT NULL
          ); PRAGMA user_version=40;`);
        }
        if (version < 41) {
          this.sql.exec(`CREATE TABLE telegram_control_replies (
            inbox_id TEXT PRIMARY KEY REFERENCES telegram_inbox(id), principal TEXT NOT NULL, bot_id TEXT NOT NULL,
            payload TEXT NOT NULL, payload_digest TEXT NOT NULL, state TEXT NOT NULL,
            observation TEXT, receipt TEXT, reason TEXT,
