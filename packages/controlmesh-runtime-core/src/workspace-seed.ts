...
export function validateWorkspaceSeed(value: unknown, expectedDigest: string, allowedFiles: readonly string[]): WorkspaceSeedManifest {
  ...
  for (const file of value.files) {
    requireThat(object(file) && Object.keys(file).every(key => ["path", "size", "sha256", "mode"].includes(key)) && typeof file.path === "string" && file.path.length <= 4096
    ...
    requireThat(file.mode === undefined || (Number.isSafeInteger(file.mode) && Number(file.mode) >= 0 && Number(file.mode) <= 0o777), "workspace_seed_mode_invalid");
...
export function prepareWorkspaceSeed(stateRoot: string, workspace: string, manifest: unknown, expectedDigest: string,
  ...
  for (const file of selected.files) {
    if (file.mode !== undefined) {
      try {
        const original = lstatSync(join(workspace, file.path));
        requireThat(original.isFile() && (original.mode & 0o7777) === file.mode, "workspace_seed_existing_conflict");
    ...
    try {
      ...
      requireThat(stat.isFile() && stat.size === file.size && (file.mode === undefined || (stat.mode & 0o7777) === file.mode), "workspace_seed_existing_conflict");
  ...
  for (const file of selected.files) stage.writeSelectedFile(authority, file.path, bytes.get(file.path)!, file.mode);
