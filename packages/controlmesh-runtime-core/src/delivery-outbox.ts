...
import type { TelegramControlReply, TelegramControlReceipt, TelegramTaskCancelAction } from "./telegram-control-reply";
...
export class DeliveryOutbox {
  ...
  telegramTaskPage(adapterId: string, message: TelegramIncomingMessage, after = "") {
    ...
    const rows = this.kernel.db.sql.query(`SELECT task_id FROM delivery_routes WHERE principal=? AND active=1
      ...
      AND json_extract(binding,'$.target.chat_id')=? AND json_extract(binding,'$.target.thread_id')=? ORDER BY task_id LIMIT 11`)
      .all(this.actor.id, adapterId, adapter.binding_digest, after, message.chat_id, message.thread_id) as { task_id: string }[];
    const tasks = rows.slice(0, 10).map(row => {
      const route = this.routeBinding(row.task_id);
      const task = this.kernel.inspect(this.actor, row.task_id);
      return { task_id: row.task_id, revision: task.revision, status: task.task.status, route_digest: route.row.digest,
  ...
  assertTelegramTaskAction(adapterId: string, callback: TelegramIncomingCallback, action: TelegramTaskCancelAction): void {
    this.current("delivery:read");
    const { row, binding } = this.routeBinding(action.task_id), adapter = this.adapters.get(adapterId);
    requireThat(adapter && adapter.binding_digest === digest({ adapter: "telegram_text.v1", adapter_id: adapterId,
      bot_id: callback.bot_id, transport: "telegram" }) && row.adapter_id === adapterId && row.adapter_digest === adapter.binding_digest
      && row.digest === action.route_digest && binding.target.transport === "telegram" && binding.target.chat_id === callback.chat_id
      && binding.target.thread_id === callback.thread_id, "telegram_management_target_changed");
    this.adapterCurrent(adapter);
    const task = this.kernel.inspect(this.actor, action.task_id);
    requireThat(task.revision === action.revision && ["waiting", "running"].includes(task.task.status), "telegram_management_task_stale");
  }
