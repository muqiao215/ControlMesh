...
const workspaceTools = [
  { name: "read_file", description: 'Read current authorized file bytes. First read example: {"request_id":"read-1","path":"PROJECT.md"}. Omit expected_sha256 on the first page when you do not yet know the hash; never use missing, null or a guessed hash for reads. Follow next_offset until eof, passing the returned sha256 as expected_sha256 on every subsequent page. A partial page does not establish a complete required read. At most 256 total workspace requests per execution.',
    properties: { request_id: requestId, path, offset: { type: "integer", minimum: 0 }, expected_sha256: { type: "string", pattern: "^[a-f0-9]{64}$", description: "Optional on the first page; required on subsequent pages. Copy the sha256 returned by the first read." } }, required: ["request_id", "path"] },
