import type { NativeAgentConfiguration } from "./native-agent-profile";
...
export class CodexRegistration {
  ...
  constructor(private readonly kernel: RuntimeKernel, private readonly cache: PreflightCache, private readonly actor: Principal,
    value: unknown, private readonly state: string, private readonly workspace: string, private readonly authorize: () => void, private readonly communicationForTask?: (taskId: string) => NativeAgentConfiguration | undefined) {
  ...
  adapter(task: TaskSnapshot, recovery = false): CodexTaskAdapter {
    ...
    const config: CodexTaskConfiguration = { ...(communication ? { communication } : {}), executable: this.config.executable, cli_version: "0.154.0", state_home: this.state,
