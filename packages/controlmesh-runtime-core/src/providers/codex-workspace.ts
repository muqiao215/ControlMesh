...
export function createCodexWorkspace(config: CodexWorkspaceConfiguration, workspace: string, directory: string, binding: string,
  lease: Lease, current: () => void, assertDispatched: () => void) {
  const journal = join(directory, "receipts"); mkdirSync(journal, { mode: 0o700 });
  const files = new NativeWorkspaceFiles({ workspace, read_files: config.read_files, tools: ["controlmesh_read_file"], journal_directory: journal, binding_digest: binding },
  ...
  return { channel, scope: files.scope };
}
export function verifyCodexWorkspace(config: CodexWorkspaceConfiguration, workspace: string, scope: Record<string, unknown>, binding: string,
  stdout: string, current: () => void, completion: unknown) {
  ...
  const files = new NativeWorkspaceFiles({ workspace, read_files: config.read_files, tools: ["controlmesh_read_file"], journal_directory: scope.journal.path,
    binding_digest: binding, retained_scope: scope }, operation => { current(); return operation(); }, current);
  const proof = files.verify(codexWorkspaceTools(stdout), config.required_reads);
