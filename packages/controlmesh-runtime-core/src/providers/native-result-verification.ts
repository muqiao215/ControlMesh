...
export class NativeResultVerification {
  ...
  constructor(private readonly store: NativeSessionStore, private readonly config: OpenCodeWorkerConfig,
    ...
    verifyCommunication?: (scope: NativeAgentScope, tools: NativeAgentToolResult[]) => Record<string, unknown>) {
    ...
    try {
      ...
      this.recheck = () => {
        authorize(); this.lock!.assertCurrent(); assertWorkspaceManifest(manifest, true); this.store.validate(verified.reference);
