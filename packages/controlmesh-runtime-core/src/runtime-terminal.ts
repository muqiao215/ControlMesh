...
const commands = ["/tasks", "/open TASK", "/new", "/model PROVIDER", "/enqueue", "/resume TEXT", "/tell TEXT", "/events", "/cancel", "/quit"];
...
export function mountRuntimeTerminal(renderer: CliRenderer, request: TerminalRequest, quit: () => void) {
  ...
  async function submit() {
    ...
    const send = async (op: string, fields: Record<string, unknown> = {}) => {
      ...
      const reply = await request(packet); if (!closed) body.content = renderRuntimeReply(reply); return result(reply);
    };
    try {
      ...
      if (!closed) footer.content = `未完成：${terminalText(error instanceof RuntimeConflict ? error.code : "response_unknown")} · 请求 ${terminalText(lastId)}\n草稿保留；先检查原任务，勿重复提交。`;
    ...
    try {
      if (selected) {
        ...
        if (object(current.task) && ["waiting", "running"].includes(String(current.task.status))) {
          ...
          try {
            const reply = await request(packet); result(reply);
