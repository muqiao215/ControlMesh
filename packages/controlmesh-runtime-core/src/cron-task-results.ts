  ...
  return kernel.db.transaction(() => {
    ...
    for (const row of rows) {
      ...
      if (task.needs_reconciliation) {
        if (attempt.state !== "uncertain") { store.updateAttemptState(attempt.attempt_id, authority, { state: "uncertain" }); changed++; }
        continue;
      }
      if (task.active_episode) {
        ...
        if (attempt.state === "initiated" && active?.state === "running") {
      ...
      if (locks[0]?.status === "uncertain") {
        store.reconcileDependencyLock(locks[0].dependency, authority,
          { attemptId: attempt.attempt_id, expectedFence: generation }, { verifiedTerminated: true, terminalState: state });
      } else {
        store.updateAttemptState(attempt.attempt_id, authority, { state, resultStatus: status });
