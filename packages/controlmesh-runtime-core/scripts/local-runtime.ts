...
try {
  ...
  const control = new LocalRuntimeControl(owned.runtime, owned.deliveries), pending = new Set<Promise<void>>();
  const output = (value: unknown) => new Promise<void>((resolve, reject) => {
    process.stdout.write(JSON.stringify(value) + "\n", error => error ? reject(error) : resolve());
  });
  const decoder = new TextDecoder("utf-8", { fatal: true });
  let buffer = "", failed: unknown;
  ...
  const interrupted = () => { void owned?.runtime.stop().finally(() => process.stdin.destroy()); };
