...
test("schema v1 upgrades transactionally without replacing tasks or their metadata", () => {
  ...
  try {
    ...
    db.sql.exec("DROP TABLE local_runs; DROP TABLE provider_checks; DROP TABLE device_assignments; DROP TABLE device_revocations; DROP TABLE execution_manifests; DROP TABLE effect_observations; DROP TABLE device_execution_records; DROP TABLE device_reconciliations; PRAGMA user_version=1");
    db.close();
    db = new RuntimeDatabase(path);
    expect(db.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 8 });
