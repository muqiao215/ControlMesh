  ...
  try {
    ...
    try {
      ...
      try {
        ...
        expect(otherDB.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 37 });
...
test("schema 31 upgrades without changing persisted task state", () => {
  ...
  try {
    ...
    db.sql.exec("DROP TABLE episode_deadlines; DROP TABLE process_output_chunks; DROP TABLE IF EXISTS telegram_conversations; DROP TABLE IF EXISTS telegram_event_aliases; DROP TABLE IF EXISTS telegram_inbox; DROP TABLE IF EXISTS telegram_poll_updates; DROP TABLE IF EXISTS telegram_polling; PRAGMA user_version=31;"); db.close();
