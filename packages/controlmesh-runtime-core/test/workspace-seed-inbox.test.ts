...
test("schema 28 upgrade adds the private seed inbox and retains existing runtime metadata", () => {
  ...
  try {
    ...
    const old = new Database(path); old.exec("DROP TABLE host_jobs; DROP TABLE backstage_events; DROP TABLE workspace_seed_files; DROP TABLE workspace_seed_transfers; PRAGMA user_version=28;"); old.close();
