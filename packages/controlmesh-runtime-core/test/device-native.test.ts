...
test("schema six upgrade preserves completed device evidence while adding durable recovery requests", async () => {
  ...
  f.workerDB.sql.exec("DROP TABLE native_agent_deliveries; DROP TABLE native_agent_calls; DROP TABLE native_mailbox_deliveries; DROP TABLE local_runs; DROP TABLE device_reconciliations; PRAGMA user_version=6");
  const upgraded = new RuntimeDatabase(join(f.root, "worker.sqlite"));
  try {
    expect(upgraded.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 10 });
