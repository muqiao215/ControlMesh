...
test("host job store upgrades, isolates owners and rejects stale concurrent writers", () => {
  ...
  try {
    a.sql.exec("DROP TABLE episode_deadlines; DROP TABLE process_output_chunks; DROP TABLE host_jobs; PRAGMA user_version=30;"); a.close(); a = new RuntimeDatabase(path);
