...
test("schema twelve upgrades without starting work or losing task identity", () => {
  ...
  f.db().sql.exec("DROP TABLE device_scheduled_work; DROP TABLE device_scheduler_leases; DROP TABLE device_assignment_generations; DROP TABLE device_native_adoptions; DROP TABLE command_reservations; PRAGMA user_version = 12;"); f.reopen();
  expect(f.db().sql.query("PRAGMA user_version").get()).toEqual({ user_version: 15 });
