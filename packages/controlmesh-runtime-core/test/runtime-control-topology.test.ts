...
test("version nineteen upgrade retains tasks and does not invent control configuration", async () => {
  ...
  try {
    f.db.sql.exec("DROP TABLE device_artifact_files; DROP TABLE topology_native_inputs; DROP TABLE topology_device_runs; ALTER TABLE topology_tasks DROP COLUMN execution_source; DROP TABLE topology_schedule_members; DROP TABLE topology_schedules; ALTER TABLE topology_tasks DROP COLUMN kind; DROP TABLE topology_runs; ALTER TABLE topology_tasks DROP COLUMN execution_id; DROP TABLE topology_completions; DROP TABLE topology_controls; PRAGMA user_version=19"); await f.reopen();
...
test("a bare terminal checkpoint cannot authorize parent completion, including after a version twenty upgrade", async () => {
  ...
  try {
    ...
    f.db.sql.exec("DROP TABLE device_artifact_files; DROP TABLE topology_native_inputs; DROP TABLE topology_device_runs; ALTER TABLE topology_tasks DROP COLUMN execution_source; DROP TABLE topology_schedule_members; DROP TABLE topology_schedules; ALTER TABLE topology_tasks DROP COLUMN kind; DROP TABLE topology_runs; ALTER TABLE topology_tasks DROP COLUMN execution_id; DROP TABLE topology_completions; PRAGMA user_version=20"); await f.reopen();
    expect(f.db.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 27 });
