...
test("schema 22 upgrade preserves native completion proof shape before introducing aggregate assignments", async () => {
  ...
  try {
    ...
    f.db.sql.exec("DROP TABLE topology_native_inputs; DROP TABLE topology_device_runs; ALTER TABLE topology_tasks DROP COLUMN execution_source; DROP TABLE topology_schedule_members; DROP TABLE topology_schedules; ALTER TABLE topology_tasks DROP COLUMN kind; PRAGMA user_version=22"); await f.restart();
