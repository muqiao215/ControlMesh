...
test("schema thirteen upgrade preserves task, event and command receipt identity without executing anything", () => {
  ...
  const before = rows(f.db); f.db.sql.exec("DROP TABLE topology_completions; DROP TABLE topology_controls; DROP TABLE topology_task_history; DROP TABLE topology_tasks; DROP TABLE team_topologies; DROP TABLE team_phases; DROP TABLE device_scheduled_work; DROP TABLE device_scheduler_leases; DROP TABLE device_assignment_generations; DROP TABLE device_native_adoptions; PRAGMA user_version=13");
