...
test("schema 21 upgrade preserves a completed run's accepted-input digest and then permits an explicit reopen", async () => {
  ...
  try {
    ...
    f.db.sql.exec("DROP TABLE topology_schedule_members; DROP TABLE topology_schedules; ALTER TABLE topology_tasks DROP COLUMN kind; DROP TABLE topology_runs; ALTER TABLE topology_tasks DROP COLUMN execution_id; PRAGMA user_version=21");
