...
test("schema 32 migration keeps the old episode expiry as its hard deadline", () => {
  ...
  try {
    ...
    const proof = kernel.claim(actor, "claim", "task", 1, 1000);
    db.sql.exec("DROP TABLE episode_deadlines; DROP TABLE IF EXISTS telegram_conversations; DROP TABLE IF EXISTS telegram_event_aliases; DROP TABLE IF EXISTS telegram_inbox; DROP TABLE IF EXISTS telegram_callbacks; DROP TABLE IF EXISTS telegram_poll_updates; DROP TABLE IF EXISTS telegram_polling; PRAGMA user_version=32;"); db.close();
