...
test("normal polling configuration is headless and does not require a webhook secret or start networking", async () => {
  const f = fixture(), identity = join(f.root, "telegram-identity.json");
  writeFileSync(identity, JSON.stringify({ bot_id: "123456", bot_username: "fixture_bot" }), { mode: 0o600 });
  writeFileSync(f.path, JSON.stringify({ ...f.config, source: { ...f.config.source, transport: "telegram" },
    delivery: { kind: "telegram_text", adapter_id: "selected-bot", bot_id: "123456", credentials_file: join(f.root, "unused-token.json") },
    inbound: { kind: "telegram_polling_text", credentials_file: identity, allowed_senders: ["777"], allowed_chats: ["777"] } }));
  const owned = openLocalRuntime(f.path), control = new LocalRuntimeControl(owned.runtime, owned.deliveries, owned.submissionIdentity, owned.inbound);
  try {
    expect(await control.handle({ id: "poll-status", op: "inbound_status" })).toMatchObject({ ok: true,
      result: { mode: "polling", polling: false, next_offset: 0, listener: null } });
