...
test("v2 database migrates atomically without losing a task or preflight decision", () => {
  const f = fixture(); f.task();
  f.db.sql.exec("DROP TABLE native_mailbox_deliveries; DROP TABLE local_runs; DROP TABLE device_assignments; DROP TABLE device_revocations; DROP TABLE execution_manifests; DROP TABLE effect_observations; DROP TABLE device_execution_records; DROP TABLE device_reconciliations; PRAGMA user_version=2");
  const reopened = new RuntimeDatabase(f.path); cleanup.push(() => reopened.close());
  expect(reopened.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 9 });
