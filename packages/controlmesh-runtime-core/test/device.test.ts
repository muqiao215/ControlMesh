...
test("schema fourteen upgrades preserve task and native adoption records without assigning or executing work", async () => {
  ...
  f.db.sql.exec("DROP TABLE topology_native_inputs; DROP TABLE topology_device_runs; ALTER TABLE topology_tasks DROP COLUMN execution_source; DROP TABLE topology_schedule_members; DROP TABLE topology_schedules; ALTER TABLE topology_tasks DROP COLUMN kind; DROP TABLE topology_runs; ALTER TABLE topology_tasks DROP COLUMN execution_id; DROP TABLE topology_completions; DROP TABLE topology_controls; DROP TABLE topology_task_history; DROP TABLE topology_tasks; DROP TABLE team_topologies; DROP TABLE team_phases; DROP TABLE device_scheduled_work; DROP TABLE device_scheduler_leases; DROP TABLE device_assignment_generations; PRAGMA user_version=14");
  const restored = new RuntimeDatabase(f.path); cleanup.push(() => restored.close());
  expect(restored.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 26 });
...
test("v2 database migrates atomically without losing a task or preflight decision", () => {
  ...
  const reopened = new RuntimeDatabase(f.path); cleanup.push(() => reopened.close());
  expect(reopened.sql.query("PRAGMA user_version").get()).toEqual({ user_version: 26 });
